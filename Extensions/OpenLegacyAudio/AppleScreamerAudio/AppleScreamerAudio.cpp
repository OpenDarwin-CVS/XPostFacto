...
IOService* AppleScreamerAudio::probe(IOService* provider, SInt32* score)
{
    ...
    *score = kIODefaultProbeScore;
	...
	// we added the sound entry in the PowerSurgePE and it should be available at this point
    if(sound) {
        OSData *tmpData;
        
        tmpData = OSDynamicCast(OSData, sound->getProperty(kModelPropName));
        if(tmpData) {
            if (tmpData->isEqualTo("PowerSurgeAudio", sizeof("PowerSurgeAudio") -1))  {
                *score = *score+1;
                debugIOLog (3, "- AppleScreamerAudio::probe");
                return(this);
            } 
			...
			if(AudioDetects) {
				...
				if (NULL != driverDMAEngine) {
					if ( layoutSawtooth == layoutID  ) {
						...
						if ( kSndHWInSense0 != ( inSense & kSndHWInSense0 ) ) {  // *BSR was kSndHWInSense1*
