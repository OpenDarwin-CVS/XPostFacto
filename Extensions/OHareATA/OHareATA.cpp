 ...
 *	OHareATA.cpp
 *	
	Defines the concrete driver for a ohare-ata controller.
...
#define kCompatibleString "ohare-ata"
...
#define kOHarePIOCycleEntries			11
#define kOHareMultiDMACycleEntries 		9

// ------ ¥¥¥ Maximum transfer modes the OHare plugin is capable of supporting ¥¥¥ ÐÐÐÐÐÐÐÐ
...
OSDefineMetaClassAndStructors(  OHareATA, MacIOATA )

//---------------------------------------------------------------------------

bool 
OHareATA::init(OSDictionary* properties)
{
	DLOG("OHareATA init start\n");
    ...
    {
        DLOG("OHareATA: super::init() failed\n");
        return false;
    }
...
bool 
OHareATA::start(IOService *provider)
{

    DLOG("OHareATA::start() begin\n");
    
    if( ! OSDynamicCast(AppleMacIODevice, provider ) )
    {
    
    	DLOG("OHareATA provider not AppleMacIODevice!\n");
    	return false;    
    ...
    if (!super::start( provider))
 	{
        DLOG("OHareATA: super::start() failed\n");
        return false;
	}

    DLOG("OHareATA::start() done\n");
 ...
 *
 ---------------------------------------------------------------------------*/
IOWorkLoop*
OHareATA::getWorkLoop() const
{

	DLOG("OHareATA::getWorkLoop\n");
 ...
 *
 ---------------------------------------------------------------------------*/
bool 
OHareATA::configureTFPointers(void)
{
    DLOG("OHareATA config TF Pointers \n");
    ...
    DLOG("OHareATA setting default timing \n");
	
	OSWriteSwapInt32(_timingConfigReg, 0, busTimings[0].cycleRegValue );

   DLOG("OHareATA configTFPointers done\n");
 ...
 *
 ---------------------------------------------------------------------------*/
IOReturn 
OHareATA::provideBusInfo( IOATABusInfo* infoOut)
{
	if( infoOut == 0)
	{
		DLOG("OHareATA nil pointer in provideBusInfo\n");
		return -1;
...
IOReturn 
OHareATA::getConfig( IOATADevConfig* configRequest, UInt32 unitNumber)
{
		...
		
	{
		...
		DLOG("OHareATA bad param in getConfig\n");
		return -1;	
	}
...
void 
OHareATA::selectIOTiming( ataUnitID unit )
{

	DLOG("ohareATA setting timing config %lx on unit %d\n", busTimings[unit].cycleRegValue, unit);
...
IOReturn
OHareATA::selectDevice( ataUnitID unit )
{
	_devIntSrc->disable();
