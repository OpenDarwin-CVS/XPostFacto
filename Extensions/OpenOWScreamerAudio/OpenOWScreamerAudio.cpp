...
OSDefineMetaClassAndStructors(OpenOWScreamerAudio, IOAudioDevice)

bool OpenOWScreamerAudio::init(OSDictionary *properties)
{
    CLOG("+OpenOWScreamerAudio::init\n");
...
void OpenOWScreamerAudio::free()
{
    CLOG("+OpenOWScreamerAudio::free\n");
    super::free();
    CLOG("-OpenOWScreamerAudio::free\n");
}

void OpenOWScreamerAudio::retain() const
{
    CLOG("+OpenOWScreamerAudio::retain\n");
    super::retain();
    CLOG("-OpenOWScreamerAudio::retain\n");
}

void OpenOWScreamerAudio::release() const
{
    CLOG("+OpenOWScreamerAudio::release\n");
    super::release();
    CLOG("-OpenOWScreamerAudio::release\n");
}

bool OpenOWScreamerAudio::start(IOService *provider)
{
    IOMemoryMap *		map;
    OpenDBDMAAudioDMAEngine 	*driverDMAEngine;
    IORegistryEntry *		perch = 0;
    IORegistryEntry *		sound = 0;
    AbsoluteTime		timerInterval;

    CLOG("+ OpenOWScreamerAudio::start\n");
    setManufacturerName("Apple");
    setDeviceName("Built-in audio controller");
    
    map = provider->mapDeviceMemoryWithIndex(OpenDBDMAAudioDMAEngine::kDBDMADeviceIndex);
    if (!map) {
        return false;
    }
    ioBase = (awacsOW_regmap_t *)map->getVirtualAddress();

    driverDMAEngine = new OpenDBDMAAudioDMAEngine;
	...
	*/
    ...
    addTimerEvent(this, &OpenOWScreamerAudio::timerCallback, timerInterval);
...
void OpenOWScreamerAudio::stop(IOService *provider)
{
    CLOG("- OpenOWScreamerAudio::stop\n");
    super::stop(provider);
    CLOG("- OpenOWScreamerAudio::stop\n");
}

void OpenOWScreamerAudio::initHardware()
{
    CLOG("+ OpenOWScreamerAudio::initHardware\n");
...
void OpenOWScreamerAudio::recalibrate()
{
    CLOG("+ OpenOWScreamerAudio::recalibrate\n");
...
IOService* OpenOWScreamerAudio::probe(IOService *provider, SInt32* score){
    IORegistryEntry *sound = 0;
    CLOG("+ OpenOWScreamerAudio::probe\n");
...
IOReturn OpenOWScreamerAudio::performPowerStateChange(IOAudioDevicePowerState oldPowerState,
...
IOReturn OpenOWScreamerAudio::performDeviceSleep(){
    IOReturn result = kIOReturnSuccess;
    
    return(result);
}

IOReturn OpenOWScreamerAudio::performDeviceWake()
{
    IOReturn result;
...
bool OpenOWScreamerAudio::createPorts(IOAudioEngine *driverDMAEngine)
{
    IOAudioPort *outputPort = 0;
    IOAudioPort *inputPort = 0;
    IOAudioPort *passThru = 0;

    CLOG("+ OpenOWScreamerAudio::createPorts\n");
    if (!driverDMAEngine) {
        return false;
    }
...
void OpenOWScreamerAudio::checkStatus(bool force)
{
...
void OpenOWScreamerAudio::timerCallback(OSObject *target, IOAudioDevice *device)
{
    OpenOWScreamerAudio *screamer;

    screamer = OSDynamicCast(OpenOWScreamerAudio, target);
    if (screamer) {
...
IOReturn OpenOWScreamerAudio::setModemSound(bool state){
    return(kIOReturnError);
}


IOReturn OpenOWScreamerAudio::callPlatformFunction( const OSSymbol * functionName,bool
...
IOReturn OpenOWScreamerAudio::setToneHardwareMuteRear(bool mute){
    IOReturn result = kIOReturnSuccess;
...
IOReturn OpenOWScreamerAudio::setToneHardwareMuteFront(bool mute){
    IOReturn result = kIOReturnSuccess;
        
    return(result);
}
    
IOReturn OpenOWScreamerAudio::setToneHardwareMuteBoomer(bool mute){
    IOReturn result = kIOReturnSuccess;
...
IOReturn OpenOWScreamerAudio::writesgs7433(UInt8 RegIndex, unsigned char RegValue )
{
    IOReturn result = kIOReturnSuccess;
...
IOReturn OpenOWScreamerAudio::readsgs7433( UInt8 RegIndex, unsigned char *RegValue)
{
     IOReturn result = kIOReturnSuccess;
...
UInt32  OpenOWScreamerAudio::sndHWGetProgOutput(void ){
    return (sndHWGetRegister(kAWACsProgOutputReg) & kAWACsProgOutputField) >> kAWACsProgOutputShift;
}

IOReturn   OpenOWScreamerAudio::sndHWSetProgOutput(UInt32 outputBits){
...
UInt32 OpenOWScreamerAudio::sndHWGetRegister(UInt32 regNum){
    return(codecControlRegister[regNum]);
}

IOReturn OpenOWScreamerAudio::sndHWSetRegister(UInt32 regNum, UInt32 value){
...
IOReturn OpenOWScreamerAudio::sndHWSetActiveInputExclusive(UInt32 input ){
...
UInt32 	 OpenOWScreamerAudio::sndHWGetActiveInputExclusive(void){
    UInt32		input;
    UInt32		inputReg;
    UInt32		pcmciaReg;
...
UInt32
OpenOWScreamerAudio::sndHWGetCurrentSampleFrame (void)
{
...
//	Calculates the PRAM volume value for stereo volume.
 UInt8 OpenOWScreamerAudio::VolumeToPRAMValue( UInt32 leftVol, UInt32 rightVol )
{ 
...
void OpenOWScreamerAudio::WritePRAMVol(  UInt32 leftVol, UInt32 rightVol  )
{
	UInt8				pramVolume;
	UInt8 				curPRAMVol;
...
IOReturn OpenOWScreamerAudio::volumeLeftChangeHandler(IOService *target, IOAudioControl *volumeControl, SInt32 oldValue, SInt32 newValue){
    IOReturn result = kIOReturnSuccess;
    OpenOWScreamerAudio *audioDevice;
    
    
    audioDevice = (OpenOWScreamerAudio *)target;
    if (audioDevice) { result = audioDevice->volumeLeftChanged(volumeControl, oldValue, newValue);}
    return result;
}

IOReturn OpenOWScreamerAudio::volumeLeftChanged(IOAudioControl *volumeControl, SInt32 oldValue, SInt32 newValue){
    IOReturn result = kIOReturnSuccess;
    SInt32 value;
...
IOReturn OpenOWScreamerAudio::volumeRightChangeHandler(IOService *target, IOAudioControl *volumeControl, SInt32 oldValue, SInt32 newValue){
    IOReturn result = kIOReturnSuccess;
    OpenOWScreamerAudio *audioDevice;
...
IOReturn OpenOWScreamerAudio::volumeRightChanged(IOAudioControl *volumeControl, SInt32 oldValue, SInt32 newValue){
    IOReturn result = kIOReturnSuccess;
    SInt32 value;
    value = newValue;
...
IOReturn OpenOWScreamerAudio::outputMuteChangeHandler(IOService *target, IOAudioControl *muteControl, SInt32 oldValue, SInt32 newValue){
    IOReturn result = kIOReturnSuccess;
    OpenOWScreamerAudio *audioDevice;
...
IOReturn OpenOWScreamerAudio::outputMuteChanged(IOAudioControl *muteControl, SInt32 oldValue, SInt32 newValue){
    IOReturn result = kIOReturnSuccess;
    SInt32 value;
    value = newValue;
...
IOReturn OpenOWScreamerAudio::gainLeftChangeHandler(IOService *target, IOAudioControl *gainControl, SInt32 oldValue, SInt32 newValue){
    IOReturn result = kIOReturnSuccess;
    OpenOWScreamerAudio *audioDevice;
...
IOReturn OpenOWScreamerAudio::gainLeftChanged(IOAudioControl *gainControl, SInt32 oldValue, SInt32 newValue){
    IOReturn result = kIOReturnSuccess;
    SInt32 value;
...
IOReturn OpenOWScreamerAudio::gainRightChangeHandler(IOService *target, IOAudioControl *gainControl, SInt32 oldValue, SInt32 newValue){
    IOReturn result = kIOReturnSuccess;
    OpenOWScreamerAudio *audioDevice;
...
IOReturn OpenOWScreamerAudio::gainRightChanged(IOAudioControl *gainControl, SInt32 oldValue, SInt32 newValue){
   IOReturn result = kIOReturnSuccess;
   SInt32 value;
...
IOReturn OpenOWScreamerAudio::passThruChangeHandler(IOService *target, IOAudioControl *passThruControl, SInt32 oldValue, SInt32 newValue){
    IOReturn result = kIOReturnSuccess;
    OpenOWScreamerAudio *audioDevice;
...
IOReturn OpenOWScreamerAudio::passThruChanged(IOAudioControl *passThruControl, SInt32 oldValue, SInt32 newValue){
...
IOReturn OpenOWScreamerAudio::setToneHardwareVolume(UInt32 volLeft, UInt32 volRight){
...
IOReturn OpenOWScreamerAudio::setToneHardwareBalance(UInt32 volLeft, UInt32 volRight){
