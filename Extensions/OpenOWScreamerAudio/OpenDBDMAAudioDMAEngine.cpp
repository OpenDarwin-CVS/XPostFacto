...
#include "OpenDBDMAAudioDMAEngine.h"
#include "OpenOWScreamerAudio.h"
...
OSDefineMetaClassAndStructors(OpenDBDMAAudioDMAEngine, super)

const int OpenDBDMAAudioDMAEngine::kDBDMADeviceIndex	= 0;
const int OpenDBDMAAudioDMAEngine::kDBDMAOutputIndex	= 1;
const int OpenDBDMAAudioDMAEngine::kDBDMAInputIndex	= 2;

bool OpenDBDMAAudioDMAEngine::init(OSDictionary	*properties,
                                 ...
                                 UInt16				numChannels)
{
	...
	CLOG("+ OpenDBDMAAudioDMAEngine::init\n");
	result = FALSE;
	...
	map = theDeviceProvider->mapDeviceMemoryWithIndex(OpenDBDMAAudioDMAEngine::kDBDMAOutputIndex);
	FailIf (NULL == map, Exit);
	ioBaseDMAOutput = (IODBDMAChannelRegisters *) map->getVirtualAddress();

	if(hasInput) {
		map = theDeviceProvider->mapDeviceMemoryWithIndex(OpenDBDMAAudioDMAEngine::kDBDMAInputIndex);
		FailIf (NULL == map, Exit);
...
void OpenDBDMAAudioDMAEngine::setSampleLatencies (UInt32 outputLatency, UInt32 inputLatency) {
	setOutputSampleLatency (outputLatency);
	setInputSampleLatency (inputLatency);
}

void OpenDBDMAAudioDMAEngine::free()
{
...
bool OpenDBDMAAudioDMAEngine::initHardware(IOService *provider)
{
	vm_offset_t					offset;
    ...
    DEBUG_IOLOG("+ OpenDBDMAAudioDMAEngine::initHardware()\n");
    ...
    interruptEventSource = IOFilterInterruptEventSource::filterInterruptEventSource(this,
                                                                               OpenDBDMAAudioDMAEngine::interruptHandler,
                                                                               OpenDBDMAAudioDMAEngine::interruptFilter,
...
void OpenDBDMAAudioDMAEngine::stop(IOService *provider)
{
    IOWorkLoop *workLoop;
    
    DEBUG3_IOLOG(" + OpenDBDMAAudioDMAEngine[%p]::stop(%p)\n", this, provider);
...
IOReturn OpenDBDMAAudioDMAEngine::performAudioEngineStart()
{
	IOPhysicalAddress			commandBufferPhys;
	IOReturn					result;

    debugIOLog(" + OpenDBDMAAudioDMAEngine::performAudioEngineStart()\n");
...
IOReturn OpenDBDMAAudioDMAEngine::restartDMA () {
	IOReturn					result;
...
IOReturn OpenDBDMAAudioDMAEngine::performAudioEngineStop()
{
    UInt16 attemptsToStop = 1000;

    debugIOLog("+ OpenDBDMAAudioDMAEngine::performAudioEngineStop()\n");
    ...
    DEBUG_IOLOG("- OpenDBDMAAudioDMAEngine::performAudioEngineStop()\n");
    return kIOReturnSuccess;
}

bool OpenDBDMAAudioDMAEngine::filterInterrupt (int index) {
...
bool OpenDBDMAAudioDMAEngine::interruptFilter(OSObject *owner, IOFilterInterruptEventSource *source)
{
    register OpenDBDMAAudioDMAEngine *dmaEngine = (OpenDBDMAAudioDMAEngine *)owner;
    bool result = true;
...
void OpenDBDMAAudioDMAEngine::interruptHandler(OSObject *owner, IOInterruptEventSource *source, int count)
{
    return;
}
...
void OpenDBDMAAudioDMAEngine::resetClipPosition (IOAudioStream *audioStream, UInt32 clipSampleFrame) {
...
IOReturn clipOpenDBDMAToOutputStreamDelayRightBalance(const void *mixBuf, void *sampleBuf, UInt32 firstSampleFrame, UInt32 numSampleFrames, const IOAudioStreamFormat *streamFormat, float* leftVolume, float* rightVolume);
...
IOReturn clipOpenDBDMAToOutputStreamiSub(const void *mixBuf, void *sampleBuf, PreviousValues *filterState, PreviousValues *filterState2, PreviousValues *phaseCompState, float *low, float *high, UInt32 firstSampleFrame, UInt32 numSampleFrames, UInt32 sampleRate, const IOAudioStreamFormat *streamFormat, SInt16 * iSubBufferMemory, UInt32 *loopCount, SInt32 *iSubBufferOffset, UInt32 iSubBufferLen, iSubAudioFormatType* iSubFormat, float* srcPhase, float* srcState, UInt32 adaptiveSampleRate);
// [3134221] aml
IOReturn clipOpenDBDMAToOutputStreamiSubDelayRight(const void *mixBuf, void *sampleBuf, PreviousValues *filterState, PreviousValues *filterState2, PreviousValues *phaseCompState, float *low, float *high, UInt32 firstSampleFrame, UInt32 numSampleFrames, UInt32 sampleRate, const IOAudioStreamFormat *streamFormat, SInt16 * iSubBufferMemory, UInt32 *loopCount, SInt32 *iSubBufferOffset, UInt32 iSubBufferLen, iSubAudioFormatType* iSubFormat, float* srcPhase, float* srcState, UInt32 adaptiveSampleRate);
IOReturn clipOpenDBDMAToOutputStreamiSubDelayRightBalance(const void *mixBuf, void *sampleBuf, PreviousValues *filterState, PreviousValues *filterState2, PreviousValues *phaseCompState, float *low, float *high, UInt32 firstSampleFrame, UInt32 numSampleFrames, UInt32 sampleRate, const IOAudioStreamFormat *streamFormat, SInt16 * iSubBufferMemory, UInt32 *loopCount, SInt32 *iSubBufferOffset, UInt32 iSubBufferLen, iSubAudioFormatType* iSubFormat, float* srcPhase, float* srcState, UInt32 adaptiveSampleRate, float* leftVolume, float* rightVolume);
IOReturn clipOpenDBDMAToOutputStreamiSubInvertRightChannel(const void *mixBuf, void *sampleBuf, PreviousValues *filterState, PreviousValues *filterState2, PreviousValues *phaseCompState, float *low, float *high, UInt32 firstSampleFrame, UInt32 numSampleFrames, UInt32 sampleRate, const IOAudioStreamFormat *streamFormat, SInt16 * iSubBufferMemory, UInt32 *loopCount, SInt32 *iSubBufferOffset, UInt32 iSubBufferLen, iSubAudioFormatType* iSubFormat, float* srcPhase, float* srcState, UInt32 adaptiveSampleRate);
IOReturn clipOpenDBDMAToOutputStreamiSubMixRightChannel(const void *mixBuf, void *sampleBuf, PreviousValues *filterState, PreviousValues *filterState2, PreviousValues *phaseCompState, float *low, float *high, UInt32 firstSampleFrame, UInt32 numSampleFrames, UInt32 sampleRate, const IOAudioStreamFormat *streamFormat, SInt16 * iSubBufferMemory, UInt32 *loopCount, SInt32 *iSubBufferOffset, UInt32 iSubBufferLen, iSubAudioFormatType* iSubFormat, float* srcPhase, float* srcState, UInt32 adaptiveSampleRate);
...
// aml 6.17.02, added dual mono mode parameter
IOReturn convertOpenDBDMAFromInputStreamWithGain(const void *sampleBuf, void *destBuf, UInt32 firstSampleFrame, UInt32 numSampleFrames, const IOAudioStreamFormat *streamFormat, float* inGainL, float* inGainR, DualMonoModeType inDualMonoMode);
IOReturn convertOpenDBDMAFromInputStreamDelayRightWithGain(const void *sampleBuf, void *destBuf, UInt32 firstSampleFrame, UInt32 numSampleFrames, const IOAudioStreamFormat *streamFormat, float* inGainL, float* inGainR, DualMonoModeType inDualMonoMode);
...
void OpenDBDMAAudioDMAEngine::setRightChanDelayInput(const bool needsRightChanDelay)  // [3336743]
{
...
// aml 5.10.02
void OpenDBDMAAudioDMAEngine::setUseSoftwareInputGain(bool inUseSoftwareInputGain) 
{
...
// aml 5.10.02
void OpenDBDMAAudioDMAEngine::setInputGainL(UInt32 inGainL) { 
...
// aml 5.10.02
void OpenDBDMAAudioDMAEngine::setInputGainR(UInt32 inGainR) { 
...
void OpenDBDMAAudioDMAEngine::setBalanceAdjust(const bool needsBalanceAdjust)  
{
...
void OpenDBDMAAudioDMAEngine::setLeftSoftVolume(UInt32 inVolume) 
{
	if (NULL != inVolume) {
...
void OpenDBDMAAudioDMAEngine::setRightSoftVolume(UInt32 inVolume) 
{ 
	if (NULL != inVolume) {
...
IOReturn OpenDBDMAAudioDMAEngine::clipOutputSamples(const void *mixBuf, void *sampleBuf, UInt32 firstSampleFrame, UInt32 numSampleFrames, const IOAudioStreamFormat *streamFormat, IOAudioStream *audioStream)
{
	IOReturn					result;
...
//				IOLog ("distance = %ld\n", distance);
		...
		if (TRUE == fNeedsPhaseInversion) {
            result = clipOpenDBDMAToOutputStreamiSubInvertRightChannel (mixBuf, sampleBuf, &filterState, &filterState2, &phaseCompState, lowFreqSamples, highFreqSamples, firstSampleFrame, numSampleFrames, sampleRate, streamFormat, (SInt16*)iSubBuffer, &iSubLoopCount, &iSubBufferOffset, iSubBufferLen, &iSubFormat, &srcPhase, &srcState, adaptiveSampleRate);
		} else if (TRUE == fNeedsRightChanMixed) {
            result = clipOpenDBDMAToOutputStreamiSubMixRightChannel (mixBuf, sampleBuf, &filterState, &filterState2, &phaseCompState, lowFreqSamples, highFreqSamples, firstSampleFrame, numSampleFrames, sampleRate, streamFormat, (SInt16*)iSubBuffer, &iSubLoopCount, &iSubBufferOffset, iSubBufferLen, &iSubFormat, &srcPhase, &srcState, adaptiveSampleRate);
		} else if (TRUE == fNeedsRightChanDelay) {	// [3134221] aml
			if (TRUE == fNeedsBalanceAdjust) {
				result = clipOpenDBDMAToOutputStreamiSubDelayRightBalance (mixBuf, sampleBuf, &filterState, &filterState2, &phaseCompState, lowFreqSamples, highFreqSamples, firstSampleFrame, numSampleFrames, sampleRate, streamFormat, (SInt16*)iSubBuffer, &iSubLoopCount, &iSubBufferOffset, iSubBufferLen, &iSubFormat, &srcPhase, &srcState, adaptiveSampleRate, (float *)&mLeftSoftVolume, (float *)&mRightSoftVolume);
			} else {
				result = clipOpenDBDMAToOutputStreamiSubDelayRight (mixBuf, sampleBuf, &filterState, &filterState2, &phaseCompState, lowFreqSamples, highFreqSamples, firstSampleFrame, numSampleFrames, sampleRate, streamFormat, (SInt16*)iSubBuffer, &iSubLoopCount, &iSubBufferOffset, iSubBufferLen, &iSubFormat, &srcPhase, &srcState, adaptiveSampleRate);
			}
		} else {
            result = clipOpenDBDMAToOutputStreamiSub (mixBuf, sampleBuf, &filterState, &filterState2, &phaseCompState, lowFreqSamples, highFreqSamples, firstSampleFrame, numSampleFrames, sampleRate, streamFormat, (SInt16*)iSubBuffer, &iSubLoopCount, &iSubBufferOffset, iSubBufferLen, &iSubFormat, &srcPhase, &srcState, adaptiveSampleRate);
		}
		...
		// this will be true on a slot load iMac that has the built in speakers enabled
		if (TRUE == fNeedsPhaseInversion) {
			result = clipOpenDBDMAToOutputStreamInvertRightChannel(mixBuf, sampleBuf, firstSampleFrame, numSampleFrames, streamFormat);
		} else if (TRUE == fNeedsRightChanMixed) {
			result = clipOpenDBDMAToOutputStreamMixRightChannel(mixBuf, sampleBuf, firstSampleFrame, numSampleFrames, streamFormat);
		} else if (TRUE == fNeedsRightChanDelay) { 	// [3134221] aml
			if (TRUE == fNeedsBalanceAdjust) {
				result = clipOpenDBDMAToOutputStreamDelayRightBalance(mixBuf, sampleBuf, firstSampleFrame, numSampleFrames, streamFormat, (float *)&mLeftSoftVolume, (float *)&mRightSoftVolume);
			} else {
				result = clipOpenDBDMAToOutputStreamDelayRight(mixBuf, sampleBuf, firstSampleFrame, numSampleFrames, streamFormat);
			}
		} else {
			result = clipOpenDBDMAToOutputStream(mixBuf, sampleBuf, firstSampleFrame, numSampleFrames, streamFormat);
		}
    }
...
IOReturn OpenDBDMAAudioDMAEngine::convertInputSamples(const void *sampleBuf, void *destBuf, UInt32 firstSampleFrame, UInt32 numSampleFrames, const IOAudioStreamFormat *streamFormat, IOAudioStream *audioStream)
{
	if (mUseSoftwareInputGain) {
		if (fNeedsRightChanDelayInput) {	// [3336743]
			return convertOpenDBDMAFromInputStreamDelayRightWithGain(sampleBuf, destBuf, firstSampleFrame, numSampleFrames, streamFormat, mInputGainLPtr, mInputGainRPtr, mInputDualMonoMode);
		} else {
			return convertOpenDBDMAFromInputStreamWithGain(sampleBuf, destBuf, firstSampleFrame, numSampleFrames, streamFormat, mInputGainLPtr, mInputGainRPtr, mInputDualMonoMode);
		}	
	} else {
		return convertOpenDBDMAFromInputStream(sampleBuf, destBuf, firstSampleFrame, numSampleFrames, streamFormat, mInputDualMonoMode);
	}
}

IOReturn OpenDBDMAAudioDMAEngine::performFormatChange(IOAudioStream *audioStream, const IOAudioStreamFormat *newFormat, const IOAudioSampleRate *newSampleRate)
{
...
IOReturn OpenDBDMAAudioDMAEngine::iSubAttachChangeHandler (IOService *target, IOAudioControl *attachControl, SInt32 oldValue, SInt32 newValue) {
    IOReturn						result;
    OpenDBDMAAudioDMAEngine *		audioDMAEngine;
    IOCommandGate *					cg;

	debug5IOLog ("+ OpenDBDMAAudioDMAEngine::iSubAttachChangeHandler (%p, %p, 0x%lx, 0x%lx)\n", target, attachControl, oldValue, newValue);

	result = kIOReturnSuccess;
	FailIf (oldValue == newValue, Exit);
    audioDMAEngine = OSDynamicCast (OpenDBDMAAudioDMAEngine, target);
...
Exit:
    debugIOLog ("- OpenDBDMAAudioDMAEngine::iSubAttachChangeHandler\n");
    return result;
}

bool OpenDBDMAAudioDMAEngine::iSubEnginePublished (OpenDBDMAAudioDMAEngine * dbdmaEngineObject, void * refCon, IOService * newService) {
	IOReturn						result;
	bool							resultCode;
    IOCommandGate *					cg;

	debug4IOLog ("+OpenDBDMAAudioDMAEngine::iSubEnginePublished (%p, %p, %p)\n", dbdmaEngineObject, (UInt32*)refCon, newService);
...
Exit:
	...
	debug5IOLog ("-OpenDBDMAAudioDMAEngine::iSubEnginePublished (%p, %p, %p), result = %d\n", dbdmaEngineObject, (UInt32 *)refCon, newService, resultCode);
	return resultCode;
}

IOReturn OpenDBDMAAudioDMAEngine::iSubCloseAction (OSObject *owner, void *arg1, void *arg2, void *arg3, void *arg4) {
    if (NULL != owner) {
        OpenDBDMAAudioDMAEngine *		audioEngine;

		debugIOLog ("+OpenDBDMAAudioDMAEngine::iSubCloseAction\n");

		audioEngine = OSDynamicCast (OpenDBDMAAudioDMAEngine, owner);
...
IOReturn OpenDBDMAAudioDMAEngine::iSubOpenAction (OSObject *owner, void *arg1, void *arg2, void *arg3, void *arg4) {
	IOReturn					result;
	bool						resultBool;

	debugIOLog ("+OpenDBDMAAudioDMAEngine::iSubOpenAction\n");
    ...
    if (NULL != owner) {
		...
		audioEngine = OSDynamicCast (OpenDBDMAAudioDMAEngine, owner);
		resultBool = audioEngine->iSubEngine->openiSub (audioEngine, NULL);
    }
...
bool OpenDBDMAAudioDMAEngine::willTerminate (IOService * provider, IOOptionBits options) {
    IOCommandGate *					cg;

	debug3IOLog ("+OpenDBDMAAudioDMAEngine[%p]::willTerminate (%p)\n", this, provider);
	...
	debug2IOLog ("-OpenDBDMAAudioDMAEngine[%p]::willTerminate - about to call super::willTerminate ()\n", this);

	return super::willTerminate (provider, options);
}

bool OpenDBDMAAudioDMAEngine::getDmaState (void )
{
	return dmaRunState;
}

IOReturn OpenDBDMAAudioDMAEngine::getAudioStreamFormat( IOAudioStreamFormat * streamFormatPtr )
{
