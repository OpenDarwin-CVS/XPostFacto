...
static long FileLoadDrivers(char *dirSpec, long plugin)
{
  ...
  if (!plugin) {
    ...
    if ((ret == 0) && ((flags & kFileTypeMask) == kFileTypeFlat)) {
      ...
      // BootX 74.1 checks for whether the cache modification time is exactly one second ahead of the Extensions directory.
      // This is useful, because it eliminates problems which can arise if the Extensions.mkext modification time is in the
      // future. But it depends on the behaviour of kextcache in 10.4, and I want to be backwards-compatible, so I'll have 
      // to use the older logic. I suppose I might be able to check whether the modification time of Extensions.mkext is
      // in the future (or perhaps at too large an offset from the Extensions directory). But I would have to figure out
      // how BootX can determine what time it is (and whether that would be directly comparable to the filesystem's sense
      // of time). Or I would have to determine a sensible maximum offset.
      // --
...
static long LoadDriverMKext(char *fileSpec)
{
  ...
  if (package->adler32 != Adler32 ((char *) &package->version, package->length - 0x10)) {
