...
// Some defines for building pre-Tiger
...
static long logs = 0;
  ...
  if(offset > raid->size) {
    printf("RAIDRead() error: offset (0x%llx) > raid->size (0x%llx)\n",
	offset, raid->size);
    return -1;
  }
  ...
  if(raid->totMembers) {
if(++logs < 15)
printf("RAIDRead(%x (curMembers: %d), %lx, %ld, 0x%llx)\n",
(unsigned) raid, raid->curMembers, buf, nbytes, offset);
    switch(raid->type) {
...
fail:
printf("ReadConcat: %ld for 0x%llx (ended 0x%llx..0x%llx)\n",
...
printf("raid: set gBootDevice to %s\n", gBootDevice);
	...
	printf("raid: master RAID device %x missing members (has %d of %d)\n",
	    (unsigned) gRAIDMaster, gRAIDMaster->curMembers, gRAIDMaster->totMembers);
      break;
    }
...
printf("raid: member size (0x%llx) -> data size (0x%llx)\n",candidate->size,
cHeader->size);
...
printf("child seq (%d) > parent's (%d) update\n",childSeq,parent->seqNum);
	...
	parent->curMembers = 0;
      ...
      case kRAIDTypeStripe:
printf("st parent->size(0x%llx) += child->size(0x%llx)\n",parent->size,child->size);
      parent->size += child->size;
...
(unsigned) member, member->type, member->curMembers, rval);
  return rval;
}
