...
static void Main(ClientInterfacePtr ciPtr)
{
  ...
  while (ret == 0) {
    ...
    if (trycache && (gBootFileType == kBlockDeviceType)) do {
      ...
      if ((ret != 0) || ((flags & kFileTypeMask) != kFileTypeFlat) || (cachetime < kerneltime)) {
        trycache = 0;
        break;
      }
      ret = GetFileInfo(gExtensionsSpec, "Extensions", &flags, &exttime);
      if ((ret == 0) && ((flags & kFileTypeMask) == kFileTypeDirectory) && (cachetime < exttime)) {
        trycache = 0;
        break;
      }

      // BootX 74.1 checks here to see whether the cachetime is exttime + 1. This is helpful in case
      // the cache time has been set in the future, which causes problems. However, it depends on
      // the behaviour of kextcache in 10.4, and I want to be backwards compatible, so I'll need
      // to leave that check out. In theory, I could (a) check for the cachetime being in the
      // future, or (b) too large an offset between cachetime and exttime. However, I would then
      // need to figure out how to get BootX to tell time (and whether that compares sensibly with
      // the filesystems's conception of time. Or I would need to figure out a sensible maximum
      // offset between cachetime and exttime.
      // --
