data 'ALIB' (0) {
	$"B929 8774"                                                                                          /* ¹)‡t */
};

data 'ALIB' (1) {
	$"03E8 03E8 03E8 03E8 03E8 03E8 03E8 03E9 03EA"                                                       /* .è.è.è.è.è.è.è.é.ê */
};

data 'ALIB' (2) {
	$"03F1 0000 0001 03E8 0000 0005 03EA 0000 0003 03EF 0000 0001 03F0 0000 0001 03EC"                    /* .ñ.....è.....ê.....ï.....ð.....ì */
	$"0000 0001"                                                                                          /* .... */
};

data 'ALIB' (3) {
	$"0000"                                                                                               /* .. */
};

data 'ALIB' (4) {
	$"0000 00AE 0000 012C 0000 0035 0000 00FF 0101 0101 01"                                               /* ...®...,...5...ÿ..... */
};

data 'ALIB' (1000) {
	$"0005 0101 0101 01"                                                                                  /* ....... */
};

data 'ALIB' (1001) {
	$"0004 0100 0301"                                                                                     /* ...... */
};

data 'ALIB' (1002) {
	$"0003 0101 01"                                                                                       /* ..... */
};

resource 'STR#' (1000) {
	{	/* array StringArray: 5 elements */
		/* [1] */
		"OK",
		/* [2] */
		"Start Mac OS X From",
		/* [3] */
		"Restart",
		/* [4] */
		"Install",
		/* [5] */
		"Install Mac OS X To"
	}
};

resource 'STR#' (1001) {
	{	/* array StringArray: 4 elements */
		/* [1] */
		"http://eshop.macsales.com/OSXCenter/",
		/* [2] */
		"",
		/* [3] */
		"Static Text",
		/* [4] */
		"For updates and information about this utility, visit Other World Computing's we"
		"b site at:"
	}
};

resource 'STR#' (1002) {
	{	/* array StringArray: 3 elements */
		/* [1] */
		"About XPostFacto",
		/* [2] */
		"XPostFacto",
		/* [3] */
		"XPostFacto Error"
	}
};

resource 'TxSt' (1000) {
	tsPlain,
	0,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	""
};

resource 'TxSt' (1001) {
	tsPlain,
	0,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	""
};

resource 'TxSt' (1002) {
	tsPlain,
	10,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	"A"
};

resource 'TxSt' (1003) {
	tsPlain,
	0,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	""
};

resource 'TxSt' (1004) {
	tsPlain,
	0,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	"A"
};

resource 'TxSt' (1005) {
	tsPlain,
	0,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	""
};

resource 'TxSt' (1006) {
	tsPlain,
	0,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	""
};

resource 'TxSt' (1007) {
	tsPlain,
	0,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	""
};

resource 'TxSt' (1008) {
	tsPlain,
	10,
	{	/* array: 3 elements */
		/* [1] */
		0x37CD,
		/* [2] */
		0x484F,
		/* [3] */
		0xDCAB
	},
	"A"
};

resource 'TxSt' (1009) {
	tsPlain,
	0,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	""
};

resource 'TxSt' (1010) {
	tsPlain,
	0,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	""
};

resource 'TxSt' (1011) {
	tsPlain,
	0,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	""
};

resource 'TxSt' (1013) {
	tsPlain,
	0,
	{	/* array: 3 elements */
		/* [1] */
		0x0,
		/* [2] */
		0x0,
		/* [3] */
		0x0
	},
	""
};

resource 'View' (1004, "MainWindow", purgeable) {
	MAThreeOhView {

	},
	{	/* array ViewArray: 9 elements */
		/* [1] */
		ViewSignatureAndClassname {
			'wind',
			1066,
			"XPFWindow",
			noID,
			enabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				50,
				/* [2] */
				50
			},
			{	/* array: 2 elements */
				/* [1] */
				320,
				/* [2] */
				362
			},
			sizeVariable,
			sizeVariable,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			AdornerListSignatureAndClassname {
				NoIdentifier,
				AdornerList,
				AdornerElementSize,
				AdornerElementSizeShift,
				DynamicArrayAllocationIncrement,
				{	/* array AdornerElementArray: 2 elements */
					/* [1] */
					AdornFirst,
					AdornerLocalObject {
						EraseAdorner
					},
					/* [2] */
					DrawView,
					AdornerLocalObject {
						DrawAdorner
					}
				}
			},
			emptyUserArea,
			Window {
				documentProc,
				noID,
				goAwayBox,
				notResizable,
				ignoreFirstClick,
				dontFreeOnClosing,
				dontDisposeOnFree,
				doesntCloseDocument,
				dontOpenWithDocument,
				dontAdaptToScreen,
				dontStagger,
				forceOnScreen,
				center,
				doesntFloat,
				doesntHideOnSuspend,
				generateActivates,
				filler,
				1002,
				2
			},
			2
		},
		/* [2] */
		ViewSignatureAndClassname {
			'clsp',
			460,
			"",
			noID,
			enabled,
			noIdle,
			{	/* array BehaviorArray: 1 elements */
				/* [1] */
				BehaviorSignatureAndClassname {
					'club',
					"TClusterBehavior",
					ClusterBehavior {
						enabled,
						noIdle
					}
				}
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				16,
				/* [2] */
				16
			},
			{	/* array: 2 elements */
				/* [1] */
				136,
				/* [2] */
				330
			},
			sizeFixed,
			sizeFixed,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			PrimaryCluster {
				mClusterHit,
				notHilited,
				notDimmed,
				sizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1009,
				dontPreferOutline,
				1000,
				2,
				noID
			},
			3
		},
		/* [3] */
		ViewSignatureAndClassname {
			'popp',
			110,
			"",
			'Star',
			enabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				26,
				/* [2] */
				14
			},
			{	/* array: 2 elements */
				/* [1] */
				23,
				/* [2] */
				192
			},
			sizeFixed,
			sizeVariable,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			Popup {
				mPopupHit,
				notHilited,
				notDimmed,
				sizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1000,
				dontPreferOutline,
				0,
				11,
				0,
				plain,
				forceLeft,
				dontUseAddResMenu,
				noID,
				1000,
				-1
			},
			NoSubviews
		},
		/* [4] */
		ViewSignatureAndClassname {
			'butn',
			96,
			"",
			'Rest',
			enabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				28,
				/* [2] */
				222
			},
			{	/* array: 2 elements */
				/* [1] */
				21,
				/* [2] */
				92
			},
			sizeFixed,
			sizeFixed,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			Button {
				mButtonHit,
				notHilited,
				dimmed,
				notSizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1000,
				dontPreferOutline,
				1000,
				3
			},
			NoSubviews
		},
		/* [5] */
		ViewSignatureAndClassname {
			'stat',
			100,
			"",
			'Mess',
			notEnabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				60,
				/* [2] */
				14
			},
			{	/* array: 2 elements */
				/* [1] */
				66,
				/* [2] */
				298
			},
			sizeFixed,
			sizeFixed,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			StaticText {
				mStaticTextHit,
				notHilited,
				notDimmed,
				notSizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1002,
				dontPreferOutline,
				autoWrap,
				eraseFirst,
				justSystem,
				1001,
				3
			},
			NoSubviews
		},
		/* [6] */
		ViewSignatureAndClassname {
			'clsp',
			460,
			"",
			'Clus',
			enabled,
			noIdle,
			{	/* array BehaviorArray: 1 elements */
				/* [1] */
				BehaviorSignatureAndClassname {
					'club',
					"TClusterBehavior",
					ClusterBehavior {
						enabled,
						noIdle
					}
				}
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				166,
				/* [2] */
				16
			},
			{	/* array: 2 elements */
				/* [1] */
				136,
				/* [2] */
				330
			},
			sizeFixed,
			sizeFixed,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			PrimaryCluster {
				mClusterHit,
				notHilited,
				dimmed,
				sizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1007,
				dontPreferOutline,
				1000,
				5,
				noID
			},
			3
		},
		/* [7] */
		ViewSignatureAndClassname {
			'popp',
			110,
			"",
			'Into',
			enabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				26,
				/* [2] */
				14
			},
			{	/* array: 2 elements */
				/* [1] */
				24,
				/* [2] */
				186
			},
			sizeFixed,
			sizeVariable,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			Popup {
				mPopupHit,
				notHilited,
				dimmed,
				sizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1000,
				dontPreferOutline,
				0,
				1,
				0,
				plain,
				forceLeft,
				dontUseAddResMenu,
				noID,
				1000,
				-1
			},
			NoSubviews
		},
		/* [8] */
		ViewSignatureAndClassname {
			'butn',
			96,
			"",
			'Inst',
			enabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				28,
				/* [2] */
				222
			},
			{	/* array: 2 elements */
				/* [1] */
				21,
				/* [2] */
				92
			},
			sizeFixed,
			sizeFixed,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			Button {
				mButtonHit,
				notHilited,
				dimmed,
				notSizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1000,
				dontPreferOutline,
				1000,
				4
			},
			NoSubviews
		},
		/* [9] */
		ViewSignatureAndClassname {
			'stat',
			100,
			"",
			'Inme',
			notEnabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				60,
				/* [2] */
				14
			},
			{	/* array: 2 elements */
				/* [1] */
				66,
				/* [2] */
				306
			},
			sizeFixed,
			sizeFixed,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			StaticText {
				mStaticTextHit,
				notHilited,
				notDimmed,
				notSizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1002,
				dontPreferOutline,
				autoWrap,
				eraseFirst,
				justSystem,
				1001,
				3
			},
			NoSubviews
		}
	}
};

resource 'View' (1005, "About Box", purgeable) {
	MAThreeOhView {

	},
	{	/* array ViewArray: 4 elements */
		/* [1] */
		ViewSignatureAndClassname {
			'wind',
			498,
			"XPFAboutBox",
			noID,
			enabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				50,
				/* [2] */
				50
			},
			{	/* array: 2 elements */
				/* [1] */
				420,
				/* [2] */
				540
			},
			sizeVariable,
			sizeVariable,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			AdornerListSignatureAndClassname {
				NoIdentifier,
				AdornerList,
				AdornerElementSize,
				AdornerElementSizeShift,
				DynamicArrayAllocationIncrement,
				{	/* array AdornerElementArray: 2 elements */
					/* [1] */
					AdornFirst,
					AdornerLocalObject {
						EraseAdorner
					},
					/* [2] */
					DrawView,
					AdornerLocalObject {
						DrawAdorner
					}
				}
			},
			emptyUserArea,
			Window {
				documentProc,
				noID,
				goAwayBox,
				notResizable,
				ignoreFirstClick,
				dontFreeOnClosing,
				dontDisposeOnFree,
				doesntCloseDocument,
				dontOpenWithDocument,
				dontAdaptToScreen,
				dontStagger,
				forceOnScreen,
				center,
				doesntFloat,
				doesntHideOnSuspend,
				generateActivates,
				filler,
				1002,
				1
			},
			3
		},
		/* [2] */
		ViewSignatureAndClassname {
			'stat',
			100,
			"",
			noID,
			notEnabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				14,
				/* [2] */
				19
			},
			{	/* array: 2 elements */
				/* [1] */
				22,
				/* [2] */
				489
			},
			sizeFixed,
			sizeRelSuperView,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			StaticText {
				mStaticTextHit,
				notHilited,
				notDimmed,
				notSizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1002,
				dontPreferOutline,
				autoWrap,
				dontEraseFirst,
				justSystem,
				1001,
				4
			},
			NoSubviews
		},
		/* [3] */
		ViewSignatureAndClassname {
			'stat',
			100,
			"",
			'urlT',
			enabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				35,
				/* [2] */
				19
			},
			{	/* array: 2 elements */
				/* [1] */
				21,
				/* [2] */
				237
			},
			sizeFixed,
			sizeFixed,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			StaticText {
				mStaticTextHit,
				notHilited,
				notDimmed,
				notSizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1008,
				dontPreferOutline,
				autoWrap,
				dontEraseFirst,
				justSystem,
				1001,
				1
			},
			NoSubviews
		},
		/* [4] */
		ViewSignatureAndClassname {
			'tevw',
			144,
			"",
			'Copy',
			enabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				59,
				/* [2] */
				19
			},
			{	/* array: 2 elements */
				/* [1] */
				345,
				/* [2] */
				502
			},
			sizeRelSuperView,
			sizeRelSuperView,
			shown,
			wantsToBeTarget,
			handlesCursor,
			doesntLetSubViewsHandleCursor,
			1,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			AdornerListSignatureAndClassname {
				NoIdentifier,
				AdornerList,
				AdornerElementSize,
				AdornerElementSizeShift,
				DynamicArrayAllocationIncrement,
				{	/* array AdornerElementArray: 2 elements */
					/* [1] */
					DrawView,
					AdornerLocalObject {
						DrawAdorner
					},
					/* [2] */
					AdornLast,
					AdornerLocalObject {
						SelectionAdorner
					}
				}
			},
			emptyUserArea,
			TEView {
				withStyle,
				autoWrap,
				dontAcceptChanges,
				dontFreeText,
				cTyping,
				unlimited,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				justSystem,
				1002,
				dontPreferOutline
			},
			NoSubviews
		}
	}
};

resource 'View' (1006, "Fatal Error", purgeable) {
	MAThreeOhView {

	},
	{	/* array ViewArray: 3 elements */
		/* [1] */
		ViewSignatureAndClassname {
			'wind',
			352,
			"XPFFatalErrorWindow",
			noID,
			enabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				50,
				/* [2] */
				50
			},
			{	/* array: 2 elements */
				/* [1] */
				233,
				/* [2] */
				478
			},
			sizeVariable,
			sizeVariable,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			AdornerListSignatureAndClassname {
				NoIdentifier,
				AdornerList,
				AdornerElementSize,
				AdornerElementSizeShift,
				DynamicArrayAllocationIncrement,
				{	/* array AdornerElementArray: 2 elements */
					/* [1] */
					AdornFirst,
					AdornerLocalObject {
						EraseAdorner
					},
					/* [2] */
					DrawView,
					AdornerLocalObject {
						DrawAdorner
					}
				}
			},
			emptyUserArea,
			Window {
				documentProc,
				noID,
				goAwayBox,
				notResizable,
				ignoreFirstClick,
				freeOnClosing,
				disposeOnFree,
				doesntCloseDocument,
				dontOpenWithDocument,
				dontAdaptToScreen,
				dontStagger,
				forceOnScreen,
				center,
				doesntFloat,
				doesntHideOnSuspend,
				generateActivates,
				filler,
				1002,
				3
			},
			2
		},
		/* [2] */
		ViewSignatureAndClassname {
			'butn',
			96,
			"",
			'Rest',
			enabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				190,
				/* [2] */
				381
			},
			{	/* array: 2 elements */
				/* [1] */
				22,
				/* [2] */
				71
			},
			sizeFixed,
			sizeFixed,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			Button {
				mButtonHit,
				notHilited,
				notDimmed,
				notSizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1000,
				dontPreferOutline,
				1000,
				1
			},
			NoSubviews
		},
		/* [3] */
		ViewSignatureAndClassname {
			'stat',
			100,
			"",
			'Erro',
			notEnabled,
			noIdle,
			{	/* array BehaviorArray: 0 elements */
			},
			MARelease13View {

			},
			{	/* array: 2 elements */
				/* [1] */
				14,
				/* [2] */
				20
			},
			{	/* array: 2 elements */
				/* [1] */
				161,
				/* [2] */
				428
			},
			sizeFixed,
			sizeFixed,
			shown,
			doesntWantToBeTarget,
			handlesCursor,
			letsSubViewsHandleCursor,
			noCursorID,
			handlesHelp,
			letsSubViewsHandleHelp,
			noHelpID,
			1,
			NoDrawingEnvironment {

			},
			NoAdorners {

			},
			emptyUserArea,
			StaticText {
				mStaticTextHit,
				notHilited,
				notDimmed,
				notSizeable,
				{	/* array: 4 elements */
					/* [1] */
					0,
					/* [2] */
					0,
					/* [3] */
					0,
					/* [4] */
					0
				},
				1004,
				dontPreferOutline,
				autoWrap,
				eraseFirst,
				justSystem,
				1001,
				3
			},
			NoSubviews
		}
	}
};

resource 'vers' (1, purgeable) {
	0x3,
	0x1,
	release,
	0x1,
	0,
	"3.0.1",
	"Last modified using Ad Lib 3.0.1"
};
